...
%>
	...
	<c:choose>
		...
		<c:when test='<%= analyticsType.equalsIgnoreCase("baidu") %>'>

			<%
			String baiduAnalyticsId = PropertiesParamUtil.getString(groupTypeSettings, request, Sites.ANALYTICS_PREFIX + analyticsType);
			%>

			<aui:input helpMessage="set-the-baidu-analytics-id-that-will-be-used-for-this-set-of-pages" label="baidu-analytics-id" name="<%= Sites.ANALYTICS_PREFIX + analyticsType %>" size="30" type="text" value="<%= baiduAnalyticsId %>" />
		</c:when>
		<c:otherwise>
